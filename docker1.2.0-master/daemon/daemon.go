...
//　创建daemon对象:需要配置参数和engine实例
// FIXME: harmonize with NewGraph()
func Newaemon(config *Config, eng *engine.Engine) (*Daemon, error) {
...
func NewDaemonFromDirectory(config *Config, eng *engine.Engine) (*Daemon, error) {
	...
	if config.Mtu == 0 {
        　//配置docker容器的最大传输单元（为０设为默认，默认先从网桥接口配置中获得，否则为1500
...
　　//处理pid文件配置，为daemon的进程pid号创建pid文件，pidfile参数指明路径
	...
	if config.Pidfile != "" {
		...
		eng.OnShutdown(func() {
            //并且为shutdown操作添加一个删除此pidfile的函数，以便在docker退出时，删除pid文件
    ...
    //检测系统支持(目前仅支持linux)以及用户权限（必须为root用户）
...
//配置工作路径，root根目录
   ...
   //创建Graph实例(管理镜像目录/文件)
    ...
    //创建TagStore管理存储镜像的仓库列表
	log.Debugf("Creating repository list")
	repositories, err := graph.NewTagStore(path.Join(config.Root, "repositories-"+driver.String()), g)
	if err != nil {
		return nil, fmt.Errorf("Couldn't create Tag store: %s", err)
	}
    //以init_networkdriver的job的形式配置docker daemon网络环境
    ...
    //创建graphdb并初始化(图形数据库，记录镜像关系)
	graphdbPath := path.Join(config.Root, "linkgraph.db")
	graph, err := graphdb.NewSqliteConn(graphdbPath)
	if err != nil {
		return nil, err
	}
//获取期望目标的dockerinit二进制文件路径
...
　　//两者不相等,说明dockerinit不在localcopy下，则复制到其下面
    ...
    //创建execdriver(执行容器的任务)：包括用户指定的驱动类型，默认为native,Docker工作路径，系统中存放dokcerinit二进制文件的路径,系统功能信息
...
　　//创建daemon实例
    ...
    //设置shutdown处理方法，接到指定信号执行关闭操作的善后操作
...
func (daemon *Daemon) checkLocaldns() error {
    ...
    //若宿主机resolve.conf文件有127.0.0.1则采用默认外在DNS地址，若其中由docker可以采用的dns地址，则采用
