package client
//每个docker命令行的命令（如pull,run）对应一个方法(用来向deemon发送对应的命令请求),如pull->CmdPull(name)
import (
	"bufio"
	"bytes"
	"encoding/base64"
...
func (cli *DockerCli) CmdPull(args ...string) error {
        //定义一个类型为flagset的cmd对象
	cmd := cli.Subcmd("pull", "NAME[:TAG]", "Pull an image or a repository from the registry")
	tag := cmd.String([]string{"#t", "#-tag"}, "", "Download tagged image in a repository")
	//第二次参数解析(针对具体的命令)，就符合tag的参数提取出出来，其余存入cmd.NArg()
        if err := cmd.Parse(args); err != nil {
		return nil
	}
       // 目前docker 支持一次下拉一个镜像
	...
	if *tag == "" {
        ...
        //首先得到镜像的Repository名称，然后由此得到镜像的host地址
	...
	if err != nil {
        ...
        //通过client实例获取与daemon server通信的认证信息
	cli.LoadConfigFile()

	// Resolve the Auth config relevant for this server
	authConfig := cli.configFile.ResolveAuthConfig(hostname)
       // 定义一个函数，根据认证信息，向daemoon发送pull命令对应的POST请求
	...
	if err := pull(authConfig); err != nil {
           //返回401说明用户没有登录，执行登录命令，成功后再次发送Pull命令请求
		if strings.Contains(err.Error(), "Status 401") {
