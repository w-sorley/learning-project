...
//请求(路由记录)和处理方法绑定，enableCors是否在http头文件中添加跨域资源共享信息
...
func createRouter(eng *engine.Engine, logging, enableCors bool, dockerVersion string) (*mux.Router, error) {
    //利用gorlla/mux创建一个全新的router实例
	r := mux.NewRouter()
	if os.Getenv("DEBUG") != "" {
		AttachProfiler(r)
	}
	m := map[string]map[string]HttpApiFunc{      //路由记录(请求到方法的映射规则)
		"GET": {
			"/_ping":                          ping,
			"/events":                         getEvents,
			"/info":                           getInfo,
...
func ServeApi(job *engine.Job) engine.Status {
	...
	var (
		protoAddrs = job.Args　
		chErrors   = make(chan error, len(protoAddrs))   //错误信息管道
	)
	activationLock = make(chan struct{})　　//用于job间的同步通道
　　//遍历协议地址，创建对应的　服务端
	for _, protoAddr := range protoAddrs {
        ...
        //通过chError建立goroutine与主进程之间的协调关系
		go func() {
...
//通知init守护进程,docker daemon启动完毕开始接受请求
