...
//运行引擎，以JOb形式管理所有任务，Docker核心
type Engine struct {
	handlers   map[string]Handler      //名字和处理函数的映射，其中hander的参数为Job指针，返回任务状态
	catchall   Handler
...
func New() *Engine {
	...
	eng := &Engine{
       ...
       //首先注册名为"commands"的Handler:以job的形式打印出所有注册的commands的名称
	eng.Register("commands", func(job *Job) Status {
       ...
       //注册默认的全局handler(保存在变量globalHandlers)
...
func (eng *Engi:ne) String() string {
...
func (eng *Engine) Job(name string, args ...string) *Job {
    ...
    //跟名称在engine注册的handler中获取对应的handler
