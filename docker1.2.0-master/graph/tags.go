...
type TagStore struct {
	path         string　　//记录镜像仓库的文件所在路径
	graph        *Graph　　　//对应的graph实例
	Repositories map[string]Repository　　　//记录镜像仓库的映射
	sync.Mutex　　//互斥锁
	// FIXME: move push/pull-related fields
	// to a helper type
	pullingPool map[string]chan struct{}　　　//记录池防止重复pull
	pushingPool map[string]chan struct{}   //同上
}
